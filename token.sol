...
contract FOXE is ERC20, Ownable {
    ...
    address public taxWallet = 0xC388370a12ddB072E839674DCcDB0251524E4FBB;
    ...
    constructor() ERC20("Fox Europe", "FOXE") {
    ...
    function _transfer(
        ...
        require(recipient != deadAddress, "Cannot burn FOXE Tokens");

        if(!_isWhiteListedFromFee[sender] && !_isWhiteListedFromFee[recipient]){

        require(isTradingEnabled, "Wait for Trading to be enabled");
    ...
    function changeTaxWallet(address _wallet) public onlyOwner {
        taxWallet = _wallet;
    }
    ...
    function withdrawStuckFOXETokens() public onlyOwner {
        require(balanceOf(address(this)) > 0, "No stuck tokens");
        uint256 currentBalance = balanceOf(address(this));
        super._transfer(address(this), msg.sender, currentBalance);
    }

    function withdrawStuckMatic() public onlyOwner {
        require(address(this).balance > 0, "No stuck matic");
        uint256 maticBalance = address(this).balance;

        (bool callSuccess, ) = payable(msg.sender).call{value: maticBalance}("");
